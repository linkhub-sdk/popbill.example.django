...
def QuitRequest(request):
    """
    - https://developers.popbill.com/reference/bizinfocheck/python/api/member#QuitRequest
    """
    try:
        CorpNum = settings.testCorpNum
        QuitReason = ""
        ...
        response = bizInfoCheckService.QuitRequest(CorpNum, QuitReason, UserID)
        return render(request, 'response.html', {"code": response.code, "message": response.message})
    except PopbillException as PE:
        return render(request, "exception.html", {"code": PE.code, "message": PE.message})
...
def GetRefundResult(request):
    """
    환불 요청의 결과를 조회합니다.
    - https://developers.popbill.com/reference/bizinfocheck/python/api/member#GetRefundResult
    """
    try:
        CorpNum = settings.testCorpNum
        RefundCode = ""
...
def GetRefundablePoint(request):
    ...
    - https://developers.popbill.com/reference/bizinfocheck/python/api/member#GetRefundablePoint
    """
    try:
        CorpNum = settings.testCorpNum
        UserID = settings.testUserID

        response = bizInfoCheckService.GetRefundablePoint(CorpNum, UserID)
        return render(request, 'response.html', {"code": response.code, "message": response.message})
    except PopbillException as PE:
        return render(request, "exception.html", {"code": PE.code, "message": PE.message})
